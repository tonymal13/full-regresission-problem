...
def calculate_var_average(var_i, n):  # вычисление среднего значения по переменной
    var_average = 0  # var c чертой var среднее

    for i in range(len(var_i)):
        var_average += var_i[i] * n[i]
        print(var_i[i], n[i])
...
def fullRegressionProblem(Xi, Yi, nij):
    ...
    m = len(nij[0])  # длина столбца

    X_avg = calculate_var_average(Xi, ni) / n  # X с чертой X среднее
    Y_avg = calculate_var_average(Yi, nj) / n  # Y с чертой Y среднее
